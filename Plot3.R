...
#Using ggplot2 to construct bar plots of the total PM2.5 emissions in Baltimore between 1999 and 2008
PlotQ3<-ggplot(NEIBAL, aes(x=factor(year), y= Emissions))+geom_bar(stat='identity')+ facet_grid(. ~ type)+labs(title = 'Values of PM2.5 Emissions per source between 1999 and 2008 in Baltimore, Maryland')+labs(x= 'year', y = 'Emissions of PM2.5 (tons)')
